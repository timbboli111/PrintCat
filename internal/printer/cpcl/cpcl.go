...
func (e *Encoder) Encode(ctx context.Context, doc document.Document, renderer render.Renderer, profile printer.PrinterProfile, options map[string]string) ([]byte, error) {
	...
	var lines []string
	lines = append(lines, fmt.Sprintf("! 0 %d %d %d %d", profile.DPI, profile.DPI, heightDots, copies))
	...
	for _, el := range doc.Elements {
		...
		case document.LineElement:
			lines = append(lines, fmt.Sprintf("LINE %d %d %d %d 1", x, y, x+w, y+h))
		case document.RectangleElement:
			lines = append(lines, fmt.Sprintf("BOX %d %d %d %d 1", x, y, x+w, y+h))
		default:
			continue
		}
	}
	...
	payload := strings.Join(lines, crlf) + crlf
	return []byte(payload), nil
}

// CPCL TEXT command: TEXT {font} {size} {x} {y} {data}
func textCommand(x, y int, content string, fontSize document.Unit, dpi int) string {
	...
	return fmt.Sprintf("TEXT 7 %d %d %d %s", sizeDots, x, y, content)
}

// CPCL EG command: EG {widthBytes} {height} {x} {y} {hexData}
// MSB-first, bit7 = leftmost pixel, 1=black, 0=white.
func imageCommand(ctx context.Context, renderer render.Renderer, x, y, w, h int, widthUm, heightUm document.Unit, idata document.ImageData, dpi int) (string, error) {
	...
	return fmt.Sprintf("EG %d %d %d %d %s", widthBytes, h, x, y, hexData), nil
}
